...
// 递归循环
const deepForRouteLabel = (routerLists: any, flag: number, arr: string[]) => {
  routerLists.forEach((val: any) => {
    if (flag > arr.length - 1) {
      return
    }
    const list = val.path.split('/')
    if (list[list.length - 1] === arr[flag]) {
      breadData.value.push(val?.meta?.label)
      if (val.children?.length > 0) {
        flag += 1
        deepForRouteLabel(val.children, flag, arr)
      }
    }
  })
}
const setBreadData = (menuArr: any): void => {
  const routeArr = menuArr.find((res: any) => res.isActive === true).path.split('/')
  const routeFilter = routeArr.filter((val: string) => val !== '' && val !== 'home')
